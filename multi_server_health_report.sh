...
################################################################################
# VM Health Report Script with SSMTP Email (Gmail)
...
check_dependencies() {
    if ! command -v wkhtmltopdf &> /dev/null; then
        log_message "ERROR" "wkhtmltopdf not found. Install: sudo apt-get install -y wkhtmltopdf"
        return 1
    fi
    if ! command -v ssmtp &> /dev/null; then
        log_message "ERROR" "ssmtp not found. Install: sudo apt-get install -y ssmtp"
        return 1
    fi
    return 0
}
...
REMOTE_DATA=$(sshpass -p "$PASSWORD" ssh \
...
<span class="metric-value" style="color:CPU_COLOR_PLACEHOLDER;">
CPU_VALUE_PLACEHOLDER%
</span>
</td>
<td style="text-align:center;">
<span class="badge CPU_STATUS_CLASS_PLACEHOLDER">
CPU_STATUS_PLACEHOLDER
</span>
</td>
</tr>
...
<td style="text-align:right;">
<span class="metric-value" style="color:MEMORY_COLOR_PLACEHOLDER;">
MEMORY_VALUE_PLACEHOLDER%
</span>
</td>
<td style="text-align:center;">
<span class="badge MEMORY_STATUS_CLASS_PLACEHOLDER">
MEMORY_STATUS_PLACEHOLDER
</span>
</td>
</tr>
...
<td style="text-align:right;">
<span class="metric-value" style="color:DISK_COLOR_PLACEHOLDER;">
DISK_VALUE_PLACEHOLDER%
</span>
</td>
<td style="text-align:center;">
<span class="badge DISK_STATUS_CLASS_PLACEHOLDER">
DISK_STATUS_PLACEHOLDER
</span>
</td>
</tr>
...
convert_html_to_pdf() {
    wkhtmltopdf --quiet --margin-top 10mm --margin-bottom 10mm --margin-left 10mm --margin-right 10mm --page-size A4 "$HTML_REPORT" "$PDF_REPORT" 2>/dev/null
    if [ $? -eq 0 ]; then
        log_message "SUCCESS" "PDF report created"
        return 0
    else
        log_message "ERROR" "Failed to convert HTML to PDF"
        return 1
    fi
}
...
send_email_with_ssmtp() {
    if ! [ -f "$PDF_REPORT" ]; then
        log_message "ERROR" "PDF report not found"
        return 1
    fi

    local pdf_base64=$(base64 -w 0 "$PDF_REPORT")
    local pdf_filename=$(basename "$PDF_REPORT")
    local email_file="/tmp/email_${TIMESTAMP}.txt"

    cat > "$email_file" << EMAILEND
From: $EMAIL_FROM
To: $EMAIL_RECIPIENT
...
Please find attached the VM Health Report for $HOSTNAME_VM.
...
EMAILEND
    ...
    if ssmtp $EMAIL_RECIPIENT < "$email_file" 2>/dev/null; then
        log_message "SUCCESS" "Email sent successfully to $EMAIL_RECIPIENT"
        rm -f "$email_file"
        return 0
    else
        log_message "ERROR" "Failed to send email via SSMTP"
        rm -f "$email_file"
        return 1
    fi
}
...
<head>
<meta charset="UTF-8">
<title>Multi Server Health Report</title>

<style>
body { font-family: Arial, sans-serif; margin: 0; padding: 20px; background: #f5f5f5; }
.container { max-width: 1000px; margin: 0 auto 30px auto; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
...
.info-box { background:#f9f9f9; padding:15px; border-left:4px solid #667eea; margin-bottom:20px; }
.info-row { display:flex; justify-content:space-between; margin:8px 0; }
.info-label { font-weight:bold; }
table { width:100%; border-collapse:collapse; margin-top:20px; }
th { background:#667eea; color:white; padding:10px; }
td { padding:10px; border:1px solid #ddd; }
.badge { display:inline-block; padding:5px 10px; color:white; border-radius:4px; }
.normal { background:#388e3c; }
.warning { background:#f57c00; }
.critical { background:#d32f2f; }
.healthy { background:#388e3c; color:white; padding:15px; text-align:center; }
.unhealthy { background:#d32f2f; color:white; padding:15px; text-align:center; }
.reason-box { margin-top:15px; padding:15px; background:#f5f5f5; }
.footer { text-align:center; padding:15px; font-size:12px; color:#666; }
.fs-normal { background:#e8f5e9; }
.fs-critical { background:#ffebee; }
</style>
...
<h1 align="center">Multi Server Health Report</h1>

EOF

while read -r SERVER <&3
do
    echo "===================================="
    echo "Server : $SERVER"

    collect_remote_data "$SERVER"

    HOSTNAME_VM=$(echo "$REMOTE_DATA" | grep "^HOSTNAME=" | cut -d= -f2)
    CPU_USAGE=$(echo "$REMOTE_DATA" | grep "^CPU=" | cut -d= -f2)
    MEMORY_USAGE=$(echo "$REMOTE_DATA" | grep "^MEMORY=" | cut -d= -f2)
    DISK_USAGE=$(echo "$REMOTE_DATA" | grep "^DISK=" | cut -d= -f2)
